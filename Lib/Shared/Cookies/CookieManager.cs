...
namespace Blazor_App.Shared
{
    public class CookieManager
    {
        ...
        public static void PushCookies(string  language, string serialized, string message, string path)
        {
            if (serialized.IsValidString() == false)
                return;
            var cookieInfo = CookUpServices.GetExtractGithubInfo(DataServiceProvider.GetHostUrl(language, false), InitCookie().Trim());
            CookUpServices.UpdateContentAsync(cookieInfo, message + "-" + language.ToString() + "-" + DateTime.Now.ToString(), serialized);
        }
